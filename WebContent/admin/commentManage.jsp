...
<script type="text/javascript">
	...
	function deleteComment(){
		var selectedRows=$("#dg").datagrid("getSelections");//选中的数据
		if(selectedRows.length==0){
			 $.messager.alert("系统提示","请选择要删除的数据！");
			 return;
		 }
		 var strIds=[];
		 for(var i=0;i<selectedRows.length;i++){//遍历多行的数据
			 strIds.push(selectedRows[i].id);
		 }
		 var ids=strIds.join(",");
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${pageContext.request.contextPath}/admin/comment/delete.do",{ids:ids},function(results){//将id字符串一同打包传给后台
						if(results.success){
	...
	function formatBlogTitle(val,row){//评论还在，但是博客已经被删除了
		if(val==null){
	...
	function formatState(val,row){//要格式化数据表格列，需要设置formatter属性，该属性是一个函数，它包含两个参数: value: 对应字段的当前列的值  record: 当前行的记录数据 
		if(val==0){
			return "待审核";
		}else if(val==1){
			return "审核通过";
